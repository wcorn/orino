...
1. GitHub Issues에서 Infra 관련 이슈를 확인한다
2. 이슈가 없으면 필요한 작업을 이슈로 생성한다
3. 이슈별 브랜치 생성 (예: `chore/<작업명>`)
4. 구현 → 커밋 → PR 생성
5. 다음 이슈로 넘어갈 때 main을 pull하고 새 브랜치를 생성한다
...
1. 현재 인프라 구성 상태를 파악한다 (ArgoCD applications, Helm 차트, IaC)
2. 열려있는 Infra 관련 GitHub 이슈를 조회해 현재 진행 가능한 과업을 정리해서 보고한다 — 특정 모듈을 미리 가정하지 않는다
3. 사용자의 지시를 기다린다 — 스스로 작업을 시작하지 않는다

## 워크트리

- 실제 구현 작업을 시작할 때 `EnterWorktree` 도구로 워크트리를 자동 생성한다
- 워크트리 이름은 `infra-{짧은해쉬}`처럼 **세션마다 고유하게** 지정한다 (예: `infra-2389ebde`)
  - 고정 이름(`infra`)을 쓰면 여러 세션이 같은 워크트리를 공유해 브랜치·커밋이 꼬인다(한 워크트리엔 브랜치 하나만 체크아웃 가능). 세션마다 고유 접미사로 격리한다
