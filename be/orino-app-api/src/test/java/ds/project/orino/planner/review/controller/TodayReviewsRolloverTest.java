...
import ds.project.orino.support.TestInstants;
 ...
 * 몇 시든 상관없다.
 */
@FixedClock(TestInstants.PRE_ROLLOVER_NOW)
class TodayReviewsRolloverTest extends ApiTestSupport {
    ...
    void calendarDayAndStudyDayDiffer() {
        assertThat(testToday(clock)).isEqualTo(LocalDate.parse(TestInstants.PRE_ROLLOVER_STUDY_DAY));
        assertThat(clock.instant().atZone(TEST_ZONE).toLocalDate())
                .isEqualTo(LocalDate.parse(TestInstants.PRE_ROLLOVER_CALENDAR_DAY));
    }
    ...
    void todayIsStudyDay() throws Exception {
        mockMvc.perform(get("/api/planner/reviews/today")
                ...
                .andExpect(jsonPath("$.data.today").value(TestInstants.PRE_ROLLOVER_STUDY_DAY));
    }

    @Test
    @DisplayName("새벽 1시에도 그 학습일 복습이 오늘 목록에 있다 — 자정을 넘겼다고 사라지지 않는다")
    void includesReviewOfCurrentStudyDay() throws Exception {
        scheduleAt(TestInstants.PRE_ROLLOVER_STUDY_DAY);
    ...
    void excludesNextStudyDay() throws Exception {
        scheduleAt(TestInstants.PRE_ROLLOVER_CALENDAR_DAY);
