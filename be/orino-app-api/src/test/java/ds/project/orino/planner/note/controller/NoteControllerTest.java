...
class NoteControllerTest extends ApiTestSupport {
    ...
    // ---------------------------------------------------------------
    // 자료 종속 노트 (material_id 있음) — GET/POST /api/notes?materialId=
    // ---------------------------------------------------------------
    @Nested
    @DisplayName("자료 노트 (materialId 지정)")
    class MaterialNotes {
        ...
        void tree_empty() throws Exception {
            mockMvc.perform(get("/api/notes").param("materialId", String.valueOf(material.getId()))
        ...
        void tree_other_material_404() throws Exception {
            ...
            mockMvc.perform(get("/api/notes").param("materialId", String.valueOf(otherMaterial.getId()))
                            .header(HttpHeaders.AUTHORIZATION, authHeader))
        ...
        void create_root() throws Exception {
            ...
            mockMvc.perform(post("/api/notes").param("materialId", String.valueOf(material.getId()))
                            .header(HttpHeaders.AUTHORIZATION, authHeader)
        ...
        @Test
        @DisplayName("POST notes - 독립 노트를 부모로 지정하면 400 (스코프 불일치)")
        void create_with_standalone_parent_400() throws Exception {
            Note standalone = noteRepository.save(
                    new Note(member.getId(), null, null, "독립 노트", 0));
            ...
            mockMvc.perform(post("/api/notes").param("materialId", String.valueOf(material.getId()))
                            ...
                            .content("""
                                    {"parentId": %d}
                                    """.formatted(standalone.getId())))
                    .andExpect(status().isBadRequest())
                    .andExpect(jsonPath("$.code").value("SP-ERR-002"));
        }

        @Test
        @DisplayName("GET note/{id} - 단건 content 포함 조회 (materialId 노출)")
        ...
        void update_move_parent() throws Exception {
            ...
            mockMvc.perform(patch("/api/notes/{id}", b.getId())
                            .header(HttpHeaders.AUTHORIZATION, authHeader)
                            .contentType(MediaType.APPLICATION_JSON)
                            .content("""
                                    {"parentId": %d}
        ...
        void update_descendant_parent_cycle_400() throws Exception {
            ...
            mockMvc.perform(patch("/api/notes/{id}", a.getId())
                            .header(HttpHeaders.AUTHORIZATION, authHeader)
                            .contentType(MediaType.APPLICATION_JSON)
                            .content("""
                                    {"parentId": %d}
        ...
        void delete_subtree_cascade() throws Exception {
            ...
            mockMvc.perform(delete("/api/notes/{id}", a.getId())
                            .header(HttpHeaders.AUTHORIZATION, authHeader))
    ...
    // ---------------------------------------------------------------
    // 공통 단건 동작 (자료/독립 무관) — content 부분수정·검증·소유
    // ---------------------------------------------------------------
    @Nested
    @DisplayName("공통 단건 동작")
    class CommonSingle {
        ...
        void detail_other_member_404() throws Exception {
            ...
            mockMvc.perform(get("/api/notes/{id}", otherNote.getId())
                            .header(HttpHeaders.AUTHORIZATION, authHeader))
                    .andExpect(status().isNotFound())
        ...
        void update_title_content() throws Exception {
            ...
            mockMvc.perform(patch("/api/notes/{id}", note.getId())
                            .header(HttpHeaders.AUTHORIZATION, authHeader)
                            .contentType(MediaType.APPLICATION_JSON)
                            .content("""
                                    {"title": "바뀐 제목",
        ...
        void delete_other_member_404() throws Exception {
            ...
            mockMvc.perform(delete("/api/notes/{id}", otherNote.getId())
                            .header(HttpHeaders.AUTHORIZATION, authHeader))
                    .andExpect(status().isNotFound());
        }
    }
    ...
    // ---------------------------------------------------------------
    // 독립 노트 (material_id NULL) — GET/POST /api/notes (materialId 생략)
    // ---------------------------------------------------------------
    @Nested
    @DisplayName("독립 노트 (materialId 생략)")
    class StandaloneNotes {

        private Note root(String title, int sortOrder) {
            return noteRepository.save(new Note(member.getId(), null, null, title, sortOrder));
        }

        private Note child(Long parentId, String title, int sortOrder) {
            return noteRepository.save(new Note(member.getId(), null, parentId, title, sortOrder));
        }

        @Test
        @DisplayName("GET notes - 독립 노트가 없으면 빈 트리")
        void tree_empty() throws Exception {
        ...
        @Test
        @DisplayName("GET notes - 자료 노트는 독립 트리에 섞이지 않는다")
        void tree_excludes_material_notes() throws Exception {
            root("독립1", 0);
            noteRepository.save(new Note(member.getId(), material.getId(), null, "자료 노트", 0));

            mockMvc.perform(get("/api/notes")
                            .header(HttpHeaders.AUTHORIZATION, authHeader))
                    .andExpect(status().isOk())
                    .andExpect(jsonPath("$.data.notes", hasSize(1)))
                    .andExpect(jsonPath("$.data.notes[0].title").value("독립1"));
        }

        @Test
        @DisplayName("GET notes - 타인 독립 노트는 트리에 포함되지 않는다")
        void tree_excludes_other_member() throws Exception {
            root("내 노트", 0);
            noteRepository.save(new Note(otherMember.getId(), null, null, "남의 노트", 0));

            mockMvc.perform(get("/api/notes")
                            .header(HttpHeaders.AUTHORIZATION, authHeader))
                    .andExpect(status().isOk())
                    .andExpect(jsonPath("$.data.notes", hasSize(1)))
                    .andExpect(jsonPath("$.data.notes[0].title").value("내 노트"));
        }

        @Test
        @DisplayName("POST notes - 루트 독립 노트 생성 (materialId null)")
        void create_root() throws Exception {
            root("기존 루트", 0);
            ...
            mockMvc.perform(post("/api/notes")
                    ...
                    .andExpect(status().isCreated())
                    .andExpect(jsonPath("$.data.title").value("새 루트"))
                    .andExpect(jsonPath("$.data.materialId").doesNotExist())
                    .andExpect(jsonPath("$.data.parentId").doesNotExist())
                    .andExpect(jsonPath("$.data.sortOrder").value(1))
                    .andExpect(jsonPath("$.data.content.type").value("doc"));
        }
        ...
        @Test
        @DisplayName("POST notes - 타인 독립 노트를 부모로 지정하면 404")
        void create_with_others_parent_404() throws Exception {
            Note othersNote = noteRepository.save(
                    new Note(otherMember.getId(), null, null, "남의 노트", 0));
            ...
            mockMvc.perform(post("/api/notes")
                            ...
                            .content("""
                                    {"parentId": %d}
                                    """.formatted(othersNote.getId())))
                    .andExpect(status().isNotFound())
                    .andExpect(jsonPath("$.code").value("SP-ERR-001"));
        }

        @Test
        @DisplayName("POST notes - 자료 노트를 부모로 지정하면 400 (스코프 불일치)")
        void create_with_material_parent_400() throws Exception {
            Note materialNote = noteRepository.save(
                    new Note(member.getId(), material.getId(), null, "자료 노트", 0));
            ...
            mockMvc.perform(post("/api/notes")
                            ...
                            .content("""
                                    {"parentId": %d}
                                    """.formatted(materialNote.getId())))
                    .andExpect(status().isBadRequest())
                    .andExpect(jsonPath("$.code").value("SP-ERR-002"));
        }

        @Test
        @DisplayName("PATCH note - sortOrder 변경이 독립 트리 정렬에 반영된다")
        void reorder_by_sortOrder() throws Exception {
            ...
            mockMvc.perform(get("/api/notes")
                            .header(HttpHeaders.AUTHORIZATION, authHeader))
                    .andExpect(status().isOk())
                    .andExpect(jsonPath("$.data.notes[0].id").value(b.getId()))
                    .andExpect(jsonPath("$.data.notes[1].id").value(a.getId()));
        }

        @Test
        @DisplayName("PATCH note - parentId·sortOrder 동시 이동이 독립 트리에 반영된다")
        void move_with_parent_and_sortOrder() throws Exception {
            Note parent = root("부모", 0);
            Note c1 = child(parent.getId(), "C1", 0);
            Note x = root("X", 1);

            mockMvc.perform(patch("/api/notes/{id}", x.getId())
                            .header(HttpHeaders.AUTHORIZATION, authHeader)
                            .contentType(MediaType.APPLICATION_JSON)
                            .content("""
            ...
            mockMvc.perform(get("/api/notes")
                            .header(HttpHeaders.AUTHORIZATION, authHeader))
                    .andExpect(jsonPath("$.data.notes", hasSize(1)))
                    .andExpect(jsonPath("$.data.notes[0].children", hasSize(2)))
                    .andExpect(jsonPath("$.data.notes[0].children[0].id").value(c1.getId()))
                    .andExpect(jsonPath("$.data.notes[0].children[1].id").value(x.getId()));
        }

        @Test
        @DisplayName("DELETE note - 독립 서브트리가 cascade 삭제된다")
        void delete_subtree_cascade() throws Exception {
            Note a = root("A", 0);
            Note b = child(a.getId(), "B", 0);
            Note sibling = root("형제", 1);
