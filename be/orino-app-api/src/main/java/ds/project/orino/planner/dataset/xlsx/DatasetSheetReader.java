 ...
 * 가계부 가져오기에도 규칙이 같은 파서가 따로 있었다. 코드를 나눠 쓰지 않은 것은
 * <b>의도한 복제</b>다 — 그쪽은 제 모듈의 오류 코드로 말했고, 두 기능이 한 파서를 공유하면
 * 한쪽의 사정이 다른 쪽 오류 메시지를 바꾼다.
 *
 * <p>여기서는 뜻을 정하지 않는다. 어느 줄이 머리글인지, A1이 우리 주소로 어떻게 옮겨지는지는
 * 다음 단계의 일이다({@link FormulaA1Reader} · 서비스). 읽기와 해석을 섞으면 「이 파일은 왜
 * 안 되나」를 물었을 때 파일 문제인지 해석 문제인지 갈라 볼 수 없다 — 가계부 쪽 파서도
 * 같은 이유로 그렇게 나뉘어 있었다.
 *
