...
/**
 * 이동수단 시트의 단건 이동시간 조회. 소유권 확인과 "정말 인접한 두 일정인가"를 여기서 본다.
 */
@Service
@Transactional(readOnly = true)
public class TravelTimeQueryService {

    private final TripRepository tripRepository;
    private final TripActivityRepository activityRepository;
    private final TravelTimeService travelTimeService;

    public TravelTimeQueryService(TripRepository tripRepository,
    ...
    public TravelTimeResponse travelTime(Long memberId, Long tripId, Long fromActivityId,
        ...
        // 보드와 어긋나면, 시트가 화면에 없는 이동을 계산하게 된다.
        List<TripActivity> ordered = activityRepository
                .findAllByTripIdAndActivityDateOrderBySortOrderAscIdAsc(tripId, date);
        return travelTimeService.travelTimeBetween(ordered, fromActivityId, toActivityId, mode);
    }
}
