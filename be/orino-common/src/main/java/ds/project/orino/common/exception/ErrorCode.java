...
public enum ErrorCode {
    ...
    // 0은 「안 정함」과 구분되지 않는다. 해제는 null이지 0이 아니다(명세 v2.2 §5.3).
    TRAVEL_BUDGET_INVALID("TRAVEL-ERR-024", "예산은 0보다 커야 합니다.", 400),
    // 하루 교체는 두 날짜가 서로 다를 때만 뜻이 있다. 같은 날짜를 받아 조용히 성공으로
    // 답하면, 화면이 잘못된 날짜를 보내고 있어도 "바꿨어요"라는 말만 돌아온다.
    TRAVEL_DAY_SWAP_SAME_DATE("TRAVEL-ERR-025", "같은 날짜끼리는 교체할 수 없습니다.", 400),
    // 남의 여행 경비도 404다 — 403이면 「그 id의 지출은 있다」가 새어나간다.
    TRAVEL_EXPENSE_NOT_FOUND("TRAVEL-ERR-026", "존재하지 않는 지출입니다.", 404),
    // 금액 없이 지출을 적을 수는 없다. 다만 원화든 외화든 하나면 된다 —
    // 제목도 분류도 없이 금액만 적고 저장하는 길이 이 기능의 핵심이다(경비 독립 §6.1).
    TRAVEL_EXPENSE_AMOUNT_REQUIRED("TRAVEL-ERR-027",
            "금액을 적어야 합니다.", 400),
    // 011과 조건은 같지만(고시표에 없는 통화) 도달하는 길이 다르다. 011은 환율 조회
    // 화면이 받고, 이 코드는 지출을 저장하다 받는다 — 화면이 열어야 할 것이 환율 다시
    // 보기가 아니라 그 입력 시트의 통화 칸이라 구분해 둔다.
    TRAVEL_EXPENSE_UNSUPPORTED_CURRENCY("TRAVEL-ERR-028",
            "지원하지 않는 통화입니다.", 400),
    ...
    // 자산을 지우는 길은 「아직 아무것도 붙지 않은 자산」에만 열려 있다. 거래·정기 항목·
    ...
    // 기준값(청약홈 인정 회차 · 대출 기준 원금)이 틀렸다. 일부만 있으면 어디서부터 셀지 모르고,
    // 미래 월·일은 기관이 보여줄 수 없는 값이다(D-18).
    LEDGER_BASELINE_INVALID("LDG-ERR-045",
            "기준값이 올바르지 않습니다. 빠진 값·음수·미래 날짜는 적을 수 없습니다.", 400),

    // 대출 — 원금은 이체로만 움직인다(§4.1). 지출·수입이 붙으면 잔여 원금과 통계가 함께 틀어진다.
    LEDGER_LOAN_TRANSFER_ONLY("LDG-ERR-039", "대출에는 이체만 붙일 수 있습니다.", 400),
    LEDGER_LOAN_INVALID("LDG-ERR-041", "대출 정보가 올바르지 않습니다. 만기·출금 계좌·상환일·금리를 확인해 주세요.", 400),
    // 대출 상환은 회차마다 원금·이자 비율이 바뀌어 금액 하나로 반복할 수 없고, 사람이 확정한다(D-17).
    // 정기 이체로도 적게 두면 상환 처리와 겹쳐 원금이 두 번 빠진다.
    LEDGER_RECURRING_LOAN("LDG-ERR-046", "대출 상환은 정기 항목으로 만들 수 없습니다. 상환 처리로 적어 주세요.", 400),
