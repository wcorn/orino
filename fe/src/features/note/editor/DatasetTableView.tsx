...
export function DatasetTableView({
  ...
  return (
    <NodeViewWrapper
      ...
      // 음수 마진(full-bleed) 없음. 예전엔 모바일 본문 패딩을 -mx-3로 상쇄해 표만 넓혔는데,
      // 왼쪽만 당겨지고 오른쪽은 그리드 거터(pr-5)가 남아 좌우가 비대칭이었고 문단과 좌측선도
      // 12px 어긋났다(#1023). 이제 본문 패딩 자체가 모바일에서 0이라 상쇄할 게 없다 —
      // 표는 다른 블록과 같은 좌측선에서 시작하고, 넘치는 열은 그리드 안에서 가로 스크롤한다.
    >
      {datasetId != null && (
        <DatasetGrid
          datasetId={datasetId}
