/**
 * 여행이 금액을 적는 법. <b>가계부에서 필요한 두 함수만 옮겨 왔다</b>(#1406 · #1407).
 *
 * <p>`features/ledger/lib/money`를 통째로 복사하지 않았다. 거기에는 부호 규칙
 * (수입 `+`, 지출 `−`)과 흐름별 글자색이 함께 있는데, 여행 경비에는 <b>지출만</b> 있어서
 * 부호를 고를 일이 없다. 안 쓰는 규칙을 들고 오면 다음 사람이 그걸 써도 되는 줄 안다.
 */
