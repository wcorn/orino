 ...
 *
 * <p>같은 이유로 <b>여기서는 공용 API 그물을 끈다</b>({@code support/test.ts}). 그물은
 * 오프라인에서도 응답을 만들어 주는데, 그러면 「망이 끊긴 것」과 「서버가 거절한 것」이
 * 구분되지 않는다 — 앞의 것은 세션을 지키고 뒤의 것은 로그아웃시키는 게 맞고, 아래 마지막
 * 테스트가 보는 것이 정확히 그 구분이다. 그물을 씌운 채로 돌리면 재발급이 500으로 보여
 * 로그인 화면으로 튕긴다(실제로 그렇게 한 번 깨졌다).
 */
test.describe("오프라인", () => {
  test.use({ stubUnmockedApi: false });
